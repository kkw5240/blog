...
바깥을 감싸는 함수(enclosing function)이 작아지고, 블록 내에서 호출하는 함수 이름을 적절히 짓는다면, 코드를 이해하기 쉬워진다.  
...
더 큰 문제는 깨어진 창문처럼 사람들이 함수에 세부사항을 점점 더 추가하게된다는 것이다.  

#### 위에서 아래로 코드 읽기: 내려가기 규칙 
코드는 위에서 아래로 이야기처럼 읽혀야 좋다.   
한 함수 다음에는 추상화 수준이 한 단계 낮은 함수가 온다.  
위에서 아래로 문단을 읽어내려 가듯이 코드를 구현하면 추상화 수준을 일관되게 유지하기 쉬워진다.  

### Switch 문 
switch 문은 본질적으로 N가지를 처리한다.  
불행하게도 switch믄을 완전히 피할 방법은 없다.  
다형적 객체를 생성하는 코드 안에서만 switch문을 사용하자. 상속 관계로 숨긴 후에는 절대로 다른 코드에 switch문을 노출하지 않도록 하자. (물론 불가피한 상황에는 어쩔 수 없긴 하다.)
...
함수 이름 정하기: 
1. 여러 단어가 쉽게 읽히는 명명법 사용
2. 여러 단어를 사용해 함수 기능을 잘 표현하는 이름 선택 
일관성 있는 문체를 사용하라. (모듈 내에서 함수 이름은 같은 문구, 명사, 동사를 사용)  
문체가 비슷하면 이야기를 순차적으로 풀어가기 쉬워진다.  

### 함수 인수 
함수에서 이상적인 인수의 개수는 0개(무항)다.  
3개 이상은 가능한 피한느 편이 좋다.  
4개 이상은 특별한 이유가 필요하다.  
특별한 이유가 있어도 사용하면 안 된다.  
다항 함수는 이해하기도 어렵고, 테스트하기에는 더더욱 어렵다.  

#### 많이 쓰는 단항 형식 
1. 인수에 질문을 던지는 경우 
2. 인수를 뭔가로 변환해 결과를 반환하는 경우 
  - 변환 함수에서 출력 인수를 사용하지 않아야 한다. 
3. 이벤트 함수(입력 인수만 받고, 출력 인수는 없음)
  - 이벤트라는 사실이 코드에 명확히 드러나야 함. 

#### 플래그 인수 
플래그 인수는 추하다.  
플래그 인수를 사용한다는 것은 함수가 한꺼번에 여러 가지를 처리한다고 대놓고 공표하는 셈이다.  

#### 이항 삼수 
인수가 2개인 함수는 1개인 함수보다 이해하기 어렵다.  
2개의 인수를 사용하는 함수는 인수를 확인하기위한 시간이 필요하다.  
이항 함수는 인수의 순서가 헷갈리는 경우도 자주 발생한다.  
